...
class Landscape:
    ...
    @classmethod
    def from_plumed(cls,datfile,barrier):
        data = np.loadtxt(datfile)
        x = np.sort(np.unique(data[:,0]))
        y = np.sort(np.unique(data[:,1]))
        z_1d = data[:,2]
        index=np.where((z_1d>=barrier))[0]
        z_1d[index]=barrier
        z = z_1d.reshape(len(y),len(x))
        #max_value = np.amax(z[np.isfinite(z[:,1])])
        #z[np.isinf(z)] = max_value
        der_x = data[:,3].reshape(len(y),len(x))
        der_y = data[:,4].reshape(len(y),len(x))
        return cls(x,y,z,(der_x,der_y))

    @classmethod
    def from_wham2d(cls,datfile,addborder=False):
        data = np.loadtxt(datfile)
        x = np.sort(np.unique(data[:,0]))
        y = np.sort(np.unique(data[:,1]))
        z = data[:,2].reshape(len(x),len(y)).T
        max_value = np.nanmax(z[z != np.inf])*2 
        z[z == np.inf] = max_value
        if addborder:
            z[-1,:] = max_value
            z[0,:] = max_value
            z[:,0] = max_value
